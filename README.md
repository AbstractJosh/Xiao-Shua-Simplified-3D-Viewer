...
**The bar across the top** holds Snap and Cut on the left, and Export docked on the right beside undo and redo. None of them describe the scene. Engaging a tool is a single click on its button; its parameters sit behind the caret beside it, in a panel that opens under the tool. Arming the cut plane opens its panel with it, because a plane you cannot position is not a tool.
...
- **STEP units are millimetres**, one scene unit to one. The app's own units are abstract, so any mapping is a convention; this is the one every mechanical CAD package opens without asking.
